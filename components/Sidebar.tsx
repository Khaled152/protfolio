...
const Sidebar: React.FC<SidebarProps> = ({ activeSection, onNavigate, isAdminUnlocked = false }) => {
  const allItems = [
    ...
    { id: NavSection.EXPERIENCE, label: 'Exp', icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z' },
    ...
    { id: NavSection.CONTACT, label: 'Link', icon: 'M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z' },
    { id: NavSection.ADMIN, label: 'Set', icon: 'M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z' },
  ];
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <nav className="hidden md:flex w-64 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-3 flex-col gap-2 shadow-sm shrink-0">
        {items.map((item) => (
          <button
            key={item.id}
      ...
      <nav className="md:hidden fixed bottom-4 left-4 right-4 h-16 bg-white/90 dark:bg-slate-800/90 backdrop-blur-lg border border-slate-200 dark:border-slate-700 rounded-2xl px-2 flex items-center justify-around shadow-2xl z-[90]">
        {items.map((item) => (
          <button
            key={item.id}
            onClick={() => onNavigate(item.id)}
